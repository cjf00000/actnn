...
class AutoPrecision:
    def __init__(self, model, schemes, bits, dims, max_bits=8, momentum=0.99,
    ...
    def before_iter(self):
        ...
        for l in range(self.L):
            self.schemes[l].inject_noise = False
    ...
    def iterate(self, det_grad, backprop):
        ...
        if self.iter == 0:
            ...
            # sum_c = 0
            for l in range(self.L):
                self.schemes[l].inject_noise = True
                grad = get_grad()
            ...
            self.schemes[l].inject_noise = True
            grad = get_grad()
            self.C[l] = ((det_grad - grad) ** 2).sum() * 4 # Hack: always use 2bit
            self.schemes[l].inject_noise = False
        ...
        self.batch_grad = self.batch_grad * momentum + (1 - momentum) * det_grad
    ...
    def refresh_bits(self):
        ...
        if self.iter > self.warmpup_iter:
            ...
            quantization_var = (self.C * 2 ** (-2 * self.bits.float())).sum().cuda()
            if quantization_var > overall_var * 0.1:
                print('ActNN Warning: Quantization variance is too large. Consider increasing number of bits.', quantization_var, overall_var)
